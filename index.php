...
if(strlen($Username) ==0 || strlen($Password) ==0){
  ...
  echo '<div class = "how1"><div class = "many"><br> 
    ...
    <input type = "submit" name = "next1" value = "Okay" class="btn btn-info btn-lg" style = "font-size:15;width: 100px;height: 50px"></form>
		...
		$result = mysql_query("select * from data WHERE uname = '$Username' and pname = '$Password' ");
  ...
  echo '<div class = "how1"><div class = "many"><br> 
    ...
    <input type = "submit" name = "next1" value = "Okay" class="btn btn-info btn-lg" style = "font-size:15;width: 100px;height: 50px">
	...
	} else
	 {
		...
		$result1=mysql_query("SELECT * FROM data WHERE uname = '$Username' and pname = '$Password'and approve = '1'");
		while($rowd=mysql_fetch_array($result1))
...
if ($rowd[9] == "EWB")
            ...
            $_SESSION["data"]=$rowd[0];
              ...
              mysql_query("INSERT INTO log
                (Username,Datelog,time,activitynya)
                VALUES
                ('$rowd[7]','$dtnow',now(),'RECRUITMENT login Accepted')
                ");
...
if ($rowd[9] == "DEPLOYMENT")
            ...
            $_SESSION["data"]=$rowd[0];
						//log control
							...
							mysql_query("INSERT INTO log
								(Username,Datelog,time,activitynya)
								VALUES
								('$rowd[7]','$dtnow',now(),'Cashier login Accepted')
								");
            ...
            //log control
              ...
              mysql_query("INSERT INTO log
                (Username,Datelog,time,activitynya)
                VALUES
                ('$rowd[7]','$dtnow',now(),'report log in identified')
                ");
            ...
            $_SESSION["data"]=$rowd[0];
						...
						//log control
							...
							mysql_query("INSERT INTO log
								(Username,Datelog,time,activitynya)
								VALUES
								('$rowd[7]','$dtnow',now(),'Admin login Accepted')
								");
            ...
            //log control
              ...
              mysql_query("INSERT INTO log
                (Username,Datelog,time,activitynya)
                VALUES
                ('$rowd[7]','$dtnow',now(),'OTHERS login Accepted')
                ");
    ...
    header("location:declaration.php");
	   ...
	   else if ($result1=mysql_query("SELECT * FROM data WHERE uname = '$Username' and pname = '$Password'and approve = '0'")); 
       {
...
//$Usertype = strtoupper($_POST['user_type']);
            ...
            else
            {
                        ...
                        $result = mysql_query("select * from data WHERE uname = '$uname' ");
                ...
                if (mysql_num_rows($result) == 0)

                    {
                              ...
                              mysql_query("INSERT INTO data
                                    (lastname,firstname,mi,contactno,emailadd,fms,uname,pname,typenya,approve,idnum)
                                    VALUES
                                    ('$lastname','$firstname','$mi','$contactno','email','$fms','$uname','$pname','DISER','0','$idno')
                                    ");
                ...
                (Username,Datelog,time,activitynya)
                VALUES
                ('$fmsnamelog','$dtnow',now(),'Account Created')
                ");
...
<form action = "" method = "POST">
          ...
          <select class="form-control cbo" name="fmsname"  data-placeholder="Select User type" style= "height:45px;width:250px" > ;       
                       ...
                       <?php                               echo '<option >Select Division Name</option> ';  
                                                          $resultpop1 =mysql_query("SELECT * FROM divisionnya");
                                                                        while($rowm1=mysql_fetch_array($resultpop1))
                                                                    {
