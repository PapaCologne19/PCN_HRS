...
session_start();
    ...
    if(isset($_POST['process'])){
        ...
        // $height_male = mysqli_real_escape_string($link, $_POST['height_male']);
        // $height_female = mysqli_real_escape_string($link, $_POST['height_female']);
        ...
        // Insert Query
        $query = "INSERT INTO mrf(type, client, location, project_title, division, ce_number, position, position_detail, np_male, 
        ...
        salary_sched, work_duration, work_days, time_sched, day_off, outlet, dt_now, date_needed, drt, rp)
        
                VALUES('$mrf_type', '$client', '$location', '$project_title', '$division', '$ce_number', '$selected_position', '$selected_other_position', '$no_of_people_male', 
        ...
        '$salary_schedule', '$work_duration', '$work_days', '$time_schedule', '$day_off', '$outlet', '$date_requested', '$date_needed', '$direct_report', '$job_position')";

        $result = $link->query($query);

        if($result){
            echo "Yey";
            header("Location: index.php?Success=Yey");
        }
